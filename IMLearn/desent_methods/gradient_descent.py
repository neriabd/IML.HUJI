...
class GradientDescent:
    ...
    callback_: Callable[[...], None], default=default_callback
        ...
        current GD iteration. Arguments are specified in the
    ...
    def __init__(self,
        ...
        tol: float, default=1e-5
            The stopping criterion. Training stops when the Euclidean
            norm of w^(t)-w^(t-1) is less than
            specified tolerance
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        following named arguments:
            ...
            function
                Value of objective function at current point, over given data
                X, y
            - grad:  ndarray of shape specified by module's
        ...
        """
        delta = np.inf
        solution = np.zeros(f.weights_.shape[0])
        best_val, best_weight = np.inf, None
        ...
        while self.tol_ < delta and t < self.max_iter_:
            ...
            f.weights = prev_weight - eta * grad
            delta = norm(f.weights - prev_weight, ord=2)
            ...
            if val < best_val:
                best_val, best_weight = val, f.weights_

            prev_weight = f.weights
            solution += prev_weight
